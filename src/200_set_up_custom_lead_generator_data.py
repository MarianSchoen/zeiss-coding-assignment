...
# - a scatter plot per feature (column) of the data set stored as pdf 
...
custom_lead_generator = folder + "data/DataScienceCodingChallenge/data/CustomerData_LeadGenerator.csv"

clg = pd.read_csv(custom_lead_generator)

# Check the info of the DataFrame => all int, or float 
clg.info()
# q_OpeningHours are dtype object, with numerical entries + some char/str. 
# As I don't know what this indicates, I'll would ask for expert knowledge. 
# For this coding challenge, I cast it to numeric. 
# Any entry that can not be casted to numeric, will be excluded
#TODO: first drop that entry, then add manipulated to it
...
# Quickly check what features are in the data set: 
# b_specialisation_* boolean features. As some samples have multiple 
# specialisations, this can not directly be treated as a one-hot-encoded variable.
...
# ASSUMPTION: 
# I assume this data set is generated by a product 
# owner, that wants to sell its product to
# retail outlets. E.g. the product would be some kind 
# medical product, and the outlets would be
# pharmacies.
...
# clg["city_identifier"] = [str(clg.loc[i,"q_2017 Total Households"]) + "-" + str(clg.loc[i, "q_2017 Total Population"]) for i in clg.index]


# NOTE: I decide not to scale/normalize any of those features, because
# 1. I use a random forest. 
# Each feature is used to split the data independently, therefore no scaling is required. 
...
# cast "b_gekauft_gesamt" to string, for categorical plotting.
ptp["b_gekauft_gesamt"] = [str(i) for i in visited_samples["b_gekauft_gesamt"]]

# I generate one plot per column, therefore ...
for a_column in visited_samples.columns:
    # skip the non numerical columns
    if a_column in ["fakeID", "b_gekauft_gesamt", "b_in_kontakt_gewesen", "city_identifier"]: 
        continue 
    ...
    # ... and store it
    file_name = folder + "results/" + clean_name + ".pdf"
...
# lines are clearly separated the corresponding test might be 
# significant. 
...
for leave_out_index in visited_samples.index: 
    # set up a random forest, with approximately 10% trees (rule of thumb)
    # On a sidenote, the forest uses bagging etc, and is therefore quite safe for overfitting.
...
# model performance on the LOOCV: 
pd.crosstab(visited_samples_sale_label, predicted_class)
# => accuracy is slightly above 50 %, which for a 2 level prediciton is same as guessing
#NOTE: I decided to use a RF for this challenge. In order to show you all aspects
# of this explorative analysis, I keep using this RF model, even though its 
# predictions are bad. 
...
pd.crosstab(visited_samples_sale_label, rf.predict(visited_samples))
